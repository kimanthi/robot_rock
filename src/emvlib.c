...
void InitTransLog_emv(int ClearFlag)
{
    ...
    else 
    {
        ...
        {
            if (EFRead(LogFd, (unsigned char *)(&gl_tTransLog[i]), 
                sizeof(TRANS_LOG)) == sizeof(TRANS_LOG)) 
                continue;
            gl_nTransNum = i;
            break;
        }
...
unsigned long GetTransLogAmt_emv(unsigned char ucPAN_SEQ, unsigned char *paucPAN, unsigned char ucPANLen)
{
    ...
    if(i<0)
    {
        for (i = MAX_TRANS_NUM-1; i>= gl_nTransNum; i--)
        {
            if (ucPAN_SEQ != gl_tTransLog[i].ucPAN_SEQ) 
                continue;
            if (gl_tTransLog[i].ucPANLen != ucPANLen) 
...
void SaveAmtToTransLog_emv(unsigned char ucPAN_SEQ, unsigned char *paucPAN, unsigned char ucPANLen, unsigned long ulAmt)
{
    ...
    Log.ulAmt = ulAmt;
    memcpy(&gl_tTransLog[gl_nTransNum], &Log, sizeof(TRANS_LOG));
    EFseek(LogFd, (long)gl_nTransNum * sizeof(TRANS_LOG), SEEK_SET);
    EFWrite(LogFd, (unsigned char *)&Log, sizeof(TRANS_LOG));
    if (++gl_nTransNum == MAX_TRANS_NUM) 
        gl_nTransNum = 0;
...
void SetCVMResult_emv(unsigned char ucCVMType)
{
    unsigned char aucCVR[3];
    int nlen = 0;
    
    memset(aucCVR,0,sizeof(aucCVR));
    EMVGetTLVData(0x9F34, aucCVR, &nlen);
    aucCVR[0] = ucCVMType;
    EMVSetTLVData(0x9F34, aucCVR, nlen);
    return ;
}
            ...
            while(2)
            {
                ...
                if(nRet)
                {
                    if(nRet == EMV_USER_CANCEL && ucPINCancelFlag == 0 && gl_ucCancelPinFlag == 1)
                    {
                        ...
                        {
                            nRet = EMV_NEXT_CVM;
                            EMVGetTLVData(0x95, ucTVR, &nTVRlen);
                            ucTVR[2] |= 0x08;
                            EMVSetTLVData(0x95, ucTVR, nTVRlen);
                        }
